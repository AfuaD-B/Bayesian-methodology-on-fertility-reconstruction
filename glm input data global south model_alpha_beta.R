...
mod_stringedu_global_south = "model {
  for (i in 1:length(period)){
    ...
    for(j in 1:length(Age)){
      ...
      ESASFR_old_1[i,j]~dnorm(ESASFR_dhs_1[i,j],prec_esasfr)T(0,)
     
      ESASFR_old_2[i,j]~dnorm(ESASFR_dhs_2[i,j],prec_esasfr)T(0,)
      
      ESASFR_old_3[i,j]~dnorm(ESASFR_dhs_3[i,j],prec_esasfr)T(0,)
      
      ESASFR_old_4[i,j]~dnorm(ESASFR_dhs_4[i,j],prec_esasfr)T(0,)
...
jagsdat_global_south$alpha=((sd(dat_global_southedu2_$SE))/(mean(dat_global_southedu2_$SE)))^2

jagsdat_global_south$beta=(sd(dat_global_southedu2_$SE))^2/(mean(dat_global_southedu2_$SE))
...
pdf("K:/project/BayesEdu/Fertility/Afua/All countries Bayesian codes/Sensitivity/ESASFR init vs data qaul glmmodel_alpha_beta.pdf", width=12,onefile = T)
...
pdf("K:/project/BayesEdu/Fertility/Afua/All countries Bayesian codes/Sensitivity/ESASFR_dhs vs data qaul glmmodel_alpha_beta.pdf", width=15,height=12,onefile = T)
for (Country_ in seq(length(Country_plots))){
  grid.arrange(Country_plots[[Country_]])  
}
dev.off()
...
pdf("K:/project/BayesEdu/Fertility/Afua/All countries Bayesian codes/Sensitivity/ASFR UN vs data qaul glmmodel_alpha_beta.pdf",width = 12, onefile = TRUE)
for (Country_ in seq(length(Country_plots))){
  grid.arrange(Country_plots[[Country_]])  
}
dev.off()
...
pdf("K:/project/BayesEdu/Fertility/Afua/All countries Bayesian codes/Sensitivity/ESTFR_dhs vs data qaul glmmodel_alpha_beta.pdf",width = 12, onefile = TRUE)
...
write_xlsx(list("BESASFR"=bayesdat_global_southesasfr,"BESTFR"=bayesdat_global_southestfr,
           ...
           path ="K:/project/BayesEdu/Fertility/Afua/All countries Bayesian codes/Sensitivity/BESFR_data qaul glmmodel_alpha_beta.xlsx", col_names=TRUE)
