...
pdf("./ESASFR init vs data qaul glmmodel_pred_prec.pdf", width=12,onefile = T)
...
pdf("./ESASFR_dhs vs data qaul glmmodel_pred_prec.pdf", width=15,height=12,onefile = T)
for (Country_ in seq(length(Country_plots))){
  grid.arrange(Country_plots[[Country_]])  
}
dev.off()
...
pdf("./ASFR UN vs data qaul glmmodel_pred_prec.pdf",width = 12, onefile = TRUE)
for (Country_ in seq(length(Country_plots))){
  grid.arrange(Country_plots[[Country_]])  
}
dev.off()
...
pdf("./ESTFR_dhs vs data qaul glmmodel_pred_prec.pdf",width = 12, onefile = TRUE)
...
write_xlsx(list("BESASFR"=bayesdat_global_southesasfr,"BESTFR"=bayesdat_global_southestfr,
           ...
           path ="./BESFR_data qaul glmmodel_pred_prec.xlsx", col_names=TRUE)
